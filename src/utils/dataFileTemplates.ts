...
export interface DataFileTemplate {
  ...
  label: string;
  columns: string[];
  /** The template's units row, one entry per column, "n.a." where none apply. */
  units: string[];
}
...
for (const t of DATA_FILE_TEMPLATES) {
  if (t.columns.length !== t.units.length) {
    throw new Error(`${t.id}: ${t.columns.length} columns but ${t.units.length} units`);
  }
}

const byId = new Map(DATA_FILE_TEMPLATES.map((t) => [t.id, t]));

/** The unit a template gives for a column, if it defines that column. */
export function templateUnitFor(template: DataFileTemplate, column: string): string | undefined {
  const i = template.columns.findIndex((c) => normalize(c) === normalize(column));
  return i === -1 ? undefined : template.units[i];
}
