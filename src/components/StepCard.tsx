...
export default function StepCard({ stepNumber, title, description, children }: StepCardProps) {
  return (
    <Paper
      p={{ base: "lg", md: "xl" }}
      radius="lg"
      ...
      <Flex direction={{ base: "column", md: "row" }} gap="lg">
        ...
        <Box style={{ flex: 1 }}>
          <Box mb="md">
            ...
            <Text style={{ color: "var(--brand-abyssal)", lineHeight: 1.625 }}>{description}</Text>
          </Box>

          {children && (
            <Box pt={8} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              {children}
            </Box>
          )}
        </Box>
