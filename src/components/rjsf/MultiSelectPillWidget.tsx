...
/**
 * A multi-select pill widget for RJSF array-of-enum fields.
 * Shows selected values as removable pills with a searchable dropdown.
 *
 * Supports ui:options:
 * - placeholder: search input placeholder (default: "Search…")
 * - maxWidth: max container width (default: 520)
 * - creatable: allow creating new values (default: false)
 * - emptyMessage: message when no results found (default: "Nothing found.")
 * - enumNames: display labels for enum values
 */
const MultiSelectPillWidget: React.FC<WidgetProps> = ({
  id,
  label,
  required,
  disabled,
  ...
  const placeholder = (options as any).placeholder ?? "Search…";
  const creatable = Boolean((options as any).creatable);
  const emptyMessage: React.ReactNode = (options as any).emptyMessage ?? "Nothing found.";
...
export default MultiSelectPillWidget;
